...
async function writeTrendFacts(rows2d, { sheetId, tabName, scriptUrl, action = 'write' } = {}) {
    ...
    try {
        if (mode === 'jsonp') {
            ...
            if (!res || res.ok !== true) {
                return Object.assign({}, base, { ok: false, error: (res && res.error) ? String(res.error) : 'JSONP write failed', mode });
            }
            return Object.assign({}, base, res, { mode, appended: res.written || res.appended || 0 });
        }
        ...
        if (!resp.ok || !parsed || parsed.ok !== true) {
            ...
            if (fb && fb.ok === true) return Object.assign({}, base, fb, { mode: 'post-json+jsonp-fallback', appended: fb.written || fb.appended || 0 });
            return Object.assign({}, base, { ok: false, error: (parsed && parsed.error) ? String(parsed.error) : `POST failed (${resp.status})`, mode });
        }
        return Object.assign({}, base, parsed, { mode, appended: parsed.written || parsed.appended || 0 });
    } catch (err) {
        try {
            const fb = await _jsonpWriteTrendFacts({ scriptUrl, action, sheetId, tabName, rows2d, timeoutMs: 15000 });
            if (fb && fb.ok === true) return Object.assign({}, base, fb, { mode: 'post-json+jsonp-fallback', appended: fb.written || fb.appended || 0 });
        } catch (_) {}
        return Object.assign({}, base, { ok: false, error: String((err && err.message) || err || 'write failed'), mode });
    }
}
...
async function googleSheetsWrite({ webAppUrl, sheetId, tabName, rows2d, verify = true }) {
    ...
    const wr = await writeTrendFacts(rows2d, { sheetId, tabName, scriptUrl: webAppUrl, action: 'write' });
    if (!wr.ok) {
