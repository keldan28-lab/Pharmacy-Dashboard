...
function _jsonpWriteTrendFacts({ scriptUrl, action, sheetId, tabName, rows2d, timeoutMs = 20000 }) {
    return new Promise((resolve, reject) => {
        ...
        const cleanup = () => {
            ...
            try { delete globalThis[cbName]; } catch (_) { globalThis[cbName] = undefined; }
            try { delete window[cbName]; } catch (_) { window[cbName] = undefined; }
            try { script.remove(); } catch (_) {}
        };
        const done = (err, data) => {
        ...
        console.log('[writeTrendFacts] JSONP start:', { tabName, rows: rows2d.length, callback: cbName, action });

        globalThis[cbName] = (resp) => { done(null, resp || {}); };
        window[cbName] = globalThis[cbName];
        try { (0, eval)(`var ${cbName} = globalThis["${cbName}"];`); } catch (_) {}

        timer = setTimeout(() => done(new Error('JSONP write timeout')), Math.max(1000, timeoutMs || 20000));
        script.onerror = function() { done(new Error('JSONP write script error')); };
        script.src = src;
        document.head.appendChild(script);
    });
}
...
async function writeTrendFacts({ tabName, rows2d, action = 'append', scriptUrl = TREND_FACTS_WEBAPP_URL, sheetId = TREND_FACTS_SHEET_ID } = {}) {
    ...
    console.log('[writeTrendFacts] mode:', mode, 'tabName:', tabName, 'rows:', rows2d.length);
...
try { window.writeTrendFacts = writeTrendFacts; } catch (_) {}
...
async function googleSheetsWrite({ webAppUrl, sheetId, tabName, rows2d, verify = true }) {
    ...
    const writer = (window.writeTrendFacts || writeTrendFacts);
    const wr = await writer({ tabName, rows2d, action: 'write', scriptUrl: webAppUrl, sheetId });
    if (!wr.ok) {
...
// ---- Spike Factor Admin (Apps Script Web App) ----
        ...
        async function adminWriteTrendFactsTestRow() {
            try {
                ...
                const rows2d = [["calculatedAt","k"], [ts,"admin-test"]];
                const writer = (window.writeTrendFacts || writeTrendFacts);
                _setTrendFactsWriteStatusSafe('Writing test row to test_tab…');
                const resp = await writer({
                    tabName: 'test_tab',
                    rows2d,
                    action: 'append',
                ...
                console.log('adminWriteTrendFactsTestRow:', resp);
                if (resp && resp.ok) _setTrendFactsWriteStatusSafe(`Saved to test_tab (${ts})`);
                else _setTrendFactsWriteStatusSafe(`Test write failed: ${(resp && resp.error) || 'unknown'}`);
                return resp;
            } catch (e) {
                _setTrendFactsWriteStatusSafe('Sheets write failed');
                console.error('adminWriteTrendFactsTestRow failed', e);
                return { ok: false, error: String((e && e.message) || e || 'write failed') };
            }
        }
